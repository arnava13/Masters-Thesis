...
Facilities with allocation ratios outside the range $[0.01, 20]$ are excluded as likely data errors or non-operating installations. Additionally, facilities are required to have at least one year with verified emissions $\geq$100 ktCO$_2$ to ensure sufficient emissions magnitude for detectable satellite signals, following the detection limits established by \cite{beirle2024}.
...
\subsection{Specification 1: TWFE with Continuous Treatment}\label{sec:methods:twfe}

The baseline two-way fixed effects specification is:
\begin{equation}
Y_{it} = \alpha_i + \gamma_t + \beta R_{it} + \mathbf{X}_{it}'\boldsymbol{\delta} + \varepsilon_{it}
\label{eq:twfe1}
\end{equation}
where:
\begin{itemize}
    \item $\alpha_i$: Facility fixed effects (absorb time-invariant unobservables)
    \item $\gamma_t$: Year fixed effects (absorb common time shocks)
...
The coefficient $\beta$ is identified from within-facility variation in allocation ratios over time, after controlling for common year effects. For the CO$_2$ outcome, a positive $\beta$ would indicate that higher allocation ratios (less policy stringency) are associated with higher verified emissions---equivalently, that policy stringency reduces CO$_2$. For the NOx outcome, a positive $\beta$ would indicate corresponding reductions in satellite-derived NOx, consistent with co-pollutant dynamics.

Standard errors are clustered at the power system region level using PyPSA-Eur network partitions \\cite{horsch2018}. These clusters are derived from k-means clustering on the European high-voltage transmission network topology, grouping electrical buses based on network connectivity and transmission capacity. The resulting regions capture economic and physical similarities between power plants: facilities in the same electrical region face correlated wholesale electricity prices, are subject to similar dispatch patterns and grid constraints, and share common market clearing mechanisms. This makes them appropriate units for clustered inference in studies of power sector behavior.

\subsection{Specification 2: TWFE with Region$\times$Year Fixed Effects}\label{sec:methods:twfe2}
...
\subsection{Specification 3: Callaway-Sant'Anna Difference-in-Differences}\label{sec:methods:csdid}
...
\subsection{Network-Based Clustering for Inference}\label{sec:methods:clustering}

Standard errors are clustered at the PyPSA-Eur power system cluster level throughout, representing a novel application of model-derived features for econometric inference. Importantly, these clusters are \textit{not} administrative regions but rather k-means clusters computed on power system features---specifically, the topology and electrical characteristics of the European high-voltage transmission network extracted from ENTSO-E data \cite{horsch2018}. The clustering algorithm groups electrical buses (substations) to minimize within-cluster transmission impedance, producing clusters that reflect the physical and economic structure of the electricity grid rather than political boundaries.

This clustering strategy is grounded in recent theoretical foundations for network cluster-robust inference. \cite{kojevnikov2023} establish that valid cluster-robust standard errors require clusters with low ``conductance''---the ratio of edges crossing cluster boundaries to total edges within clusters. Intuitively, observations within a cluster should be more correlated with each other than with observations in other clusters. Power system network clusters satisfy this requirement by construction: the k-means objective minimizes within-cluster electrical distance (transmission impedance), which directly corresponds to minimizing cross-cluster connections in the network graph. Facilities in the same electrical cluster face:
\begin{itemize}
    \item \textbf{Correlated wholesale prices}: Shared market clearing and limited cross-cluster transmission capacity mean prices co-move within clusters
    \item \textbf{Similar dispatch patterns}: Grid constraints and congestion patterns affect which plants are called to generate
    \item \textbf{Common demand shocks}: Regional electricity demand fluctuations propagate through the local network
\end{itemize}

I use 128 clusters, a standard resolution in the PyPSA-Eur literature for EU-wide analyses that balances geographic granularity with sufficient observations per cluster for reliable inference. This represents a novel application of model-derived clustering for econometric inference: rather than using geographic proximity, administrative boundaries, or data-driven clustering on outcome variables, I use clusters computed from features of an external domain-specific model (the power system network) that captures the economically-relevant correlation structure.

Let $r \in \{1, \ldots, R\}$ index clusters. The cluster-robust variance estimator for the TWFE coefficient is:
\begin{equation}
...
\begin{enumerate}
    ...
    \item Effects on verified CO$_2$ emissions (EU ETS) --- TWFE Specifications 1 and 2
    \item Effects on satellite-derived NOx emission proxy --- TWFE Specifications 1 and 2
...
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lcccc}
\toprule
& \multicolumn{2}{c}{Verified CO$_2$ (ktCO$_2$/yr)} & \multicolumn{2}{c}{Satellite NOx (kg/s)} \\
...
\botrule
\end{tabular*}
\footnotetext{Note: Standard errors clustered by PyPSA-Eur k-means cluster in parentheses. Controls include capacity (MW), fuel shares (coal, gas), and AlphaEarth embeddings (64 dimensions). * $p<0.10$, ** $p<0.05$, *** $p<0.01$.}
\end{table}
...
I thank my thesis advisors for guidance throughout this project. Computational resources were provided by Google Earth Engine.
...
\item \textbf{Code availability}: Analysis code available upon request
\end{itemize}
