...
\subsection{Why Callaway-Sant'Anna Is Not Feasible}\label{sec:methods:csdid}

I initially planned to complement the TWFE analysis with the Callaway and Sant'Anna \cite{callaway2021} estimator, which addresses potential bias in standard TWFE when treatment timing varies across units and treatment effects are heterogeneous. However, this approach proved infeasible due to the structure of the data.

The CS-DiD estimator requires observing units \textit{before} they first receive treatment to establish parallel pre-trends. In my setting, defining treatment as allocation ratio $R_{it} < 1$, the ``cohort'' is the first year a facility crosses this threshold. The EU ETS has operated since 2005, and by the start of my panel (2018), the vast majority of facilities that would ever face allocation shortfalls were already in that state:

\begin{itemize}
    \item Of 457 ever-treated facilities, \textbf{386 (84.5\%)} were already treated in 2018
    \item Only 71 facilities (15.5\%) first crossed the threshold during 2019--2023
    \item After excluding treatment reversers ($\sim$25\% of facilities), later cohorts have $<$10 units each
\end{itemize}

With 2018 as both the first panel year and the dominant cohort, there are no pre-treatment observations for most treated units. The CS-DiD estimator cannot identify treatment effects without this variation. Extending the panel backward to 2013--2017 (EU ETS Phase 3) could enable event-study analysis in future work.

I therefore rely on TWFE with continuous treatment, which does not require discrete cohort definitions and exploits within-facility variation in allocation ratios over time.
...
\textbf{Sample Definitions.} ETS CO$_2$ analysis uses the full panel of facilities with valid verified emissions. Satellite NOx analysis \textit{always} applies detection limit filtering---observations below the detection threshold are excluded. I estimate NOx specifications at two detection limits: (1) a permissive threshold of $\geq 0.03$ kg/s, which maximizes sample size; and (2) a conservative threshold of $\geq 0.11$ kg/s following \citet{beirle2019}, which restricts to higher-quality signals. The conservative sample serves as a robustness check.

\begin{table}[h]
\caption{Summary of Econometric Specifications}\label{tab:specs}%
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}cllll}
\toprule
Spec & Outcome & Sample & Embedding & Estimator \\
\midrule
1 & ETS CO$_2$ & Full (521 facilities) & None & TWFE \\
2 & Satellite NOx & DL $\geq 0.03$ kg/s & PCA (10 dims) & TWFE \\
3 & Satellite NOx & DL $\geq 0.03$ kg/s & PLS (10 dims) & TWFE \\
4 & Satellite NOx & DL $\geq 0.11$ kg/s & PCA (10 dims) & TWFE \\
5 & Satellite NOx & DL $\geq 0.11$ kg/s & PLS (10 dims) & TWFE \\
\botrule
\end{tabular*}
\footnotetext{Note: ETS CO$_2$ uses no embeddings because geographic context does not affect administrative data measurement. For satellite NOx, PCA provides an unsupervised (outcome-agnostic) projection while PLS provides a supervised projection trained on facility-level mean NOx to ensure causal validity (Section~\ref{sec:methods:embeddings}). NOx analysis \textit{never} uses unfiltered samples---all NOx specifications require detection limit filtering. All specifications use Facility + Region$\times$Year fixed effects and cluster standard errors by NUTS2 region.}
\end{table}
...
\begin{enumerate}
    ...
    \item Estimates for all five specifications (Table~\ref{tab:specs})
    \item Comparison of CO$_2$ and NOx responses: agreement in sign and magnitude
    \item Stability of results across embedding strategies (PCA vs PLS) and detection limits
\end{enumerate}
...
\botrule
\end{tabular*}
\footnotetext{Note: Standard errors clustered by NUTS2 region in parentheses. ETS CO$_2$ controls include capacity (MW) and fuel shares (coal, gas). Satellite NOx controls additionally include AlphaEarth embeddings (10 PCA or PLS components). NOx sample restricted to observations above detection limit ($\geq 0.03$ kg/s for main specification). * $p<0.10$, ** $p<0.05$, *** $p<0.01$.}
\end{table}
...
\begin{itemize}
    \item Sensitivity to NOx detection limit (conservative 0.11 kg/s vs permissive 0.03 kg/s)
    \item Embedding reduction method (PCA vs PLS)
    \item Split-sample heterogeneity analysis by:
    \begin{itemize}
        \item Electricity sector (activity codes 1, 20) vs other sectors
        \item Urbanization (urban vs rural facilities)
        \item Dominant fuel type (coal, gas, oil, biomass)
        \item Country (top 5 by facility count)
        \item PyPSA-Eur grid clusters (electricity sector only, top 5 by facility count)
    \end{itemize}
    \item Continuous interaction terms to test whether treatment effects vary with:
    \begin{itemize}
        \item Fuel composition (treatment $\times$ share\_coal, treatment $\times$ share\_gas, etc.)
        \item Facility capacity (treatment $\times$ capacity, standardized)
        \item Urbanization (treatment $\times$ urban, binary)
    \end{itemize}
\end{itemize}
...
\textbf{Event-study analysis.}
The Callaway-Sant'Anna \cite{callaway2021} estimator would provide a valuable robustness check through event-study plots and formal pre-trend tests. However, as discussed in Section~\ref{sec:methods:csdid}, this approach is infeasible with the current panel (2018--2023) because 84.5\% of ever-treated facilities were already treated in the first panel year. Extending the panel backward to include EU ETS Phase 3 (2013--2017) would capture the actual first-treatment timing for most facilities, enabling event-study analysis with proper pre-treatment observations. This represents a natural extension for future work.
